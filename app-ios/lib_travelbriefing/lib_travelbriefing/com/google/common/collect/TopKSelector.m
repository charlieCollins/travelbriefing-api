...
//  source: ../target/dependency/com/google/common/collect/TopKSelector.java
//
...
- (void)offerWithId:(id)elem {
  if (k_ == 0) {
    ...
    IOSObjectArray_Set(nil_chk(buffer_), 0, elem);
    JreStrongAssign(&threshold_, elem);
    bufferSize_ = 1;
  }
  else if (bufferSize_ < k_) {
    IOSObjectArray_Set(nil_chk(buffer_), bufferSize_++, elem);
...
- (id<JavaUtilList>)topK {
  ...
  if (bufferSize_ > k_) {
    ...
    bufferSize_ = k_;
    JreStrongAssign(&threshold_, IOSObjectArray_Get(buffer_, k_ - 1));
  }
...
ComGoogleCommonCollectTopKSelector *ComGoogleCommonCollectTopKSelector_leastWithInt_withJavaUtilComparator_(jint k, id<JavaUtilComparator> comparator) {
  ComGoogleCommonCollectTopKSelector_initialize();
  return create_ComGoogleCommonCollectTopKSelector_initWithJavaUtilComparator_withInt_(comparator, k);
}
...
ComGoogleCommonCollectTopKSelector *ComGoogleCommonCollectTopKSelector_greatestWithInt_withJavaUtilComparator_(jint k, id<JavaUtilComparator> comparator) {
  ComGoogleCommonCollectTopKSelector_initialize();
  return create_ComGoogleCommonCollectTopKSelector_initWithJavaUtilComparator_withInt_([((ComGoogleCommonCollectOrdering *) nil_chk(ComGoogleCommonCollectOrdering_fromWithJavaUtilComparator_(comparator))) reverse], k);
}
...
void ComGoogleCommonCollectTopKSelector_initWithJavaUtilComparator_withInt_(ComGoogleCommonCollectTopKSelector *self, id<JavaUtilComparator> comparator, jint k) {
  NSObject_init(self);
  JreStrongAssign(&self->comparator_, ComGoogleCommonBasePreconditions_checkNotNullWithId_withId_(comparator, @"comparator"));
  self->k_ = k;
  ComGoogleCommonBasePreconditions_checkArgumentWithBoolean_withNSString_withInt_(k >= 0, @"k must be nonnegative, was %s", k);
  JreStrongAssign(&self->buffer_, [IOSObjectArray arrayWithLength:k * 2 type:NSObject_class_()]);
  self->bufferSize_ = 0;
  JreStrongAssign(&self->threshold_, nil);
}
...
void ComGoogleCommonCollectTopKSelector_trim(ComGoogleCommonCollectTopKSelector *self) {
  ...
  JreStrongAssign(&self->threshold_, IOSObjectArray_Get(nil_chk(self->buffer_), minThresholdPosition));
  for (jint i = minThresholdPosition + 1; i < self->k_; i++) {
    if ([((id<JavaUtilComparator>) nil_chk(self->comparator_)) compareWithId:IOSObjectArray_Get(self->buffer_, i) withId:self->threshold_] > 0) {
      JreStrongAssign(&self->threshold_, IOSObjectArray_Get(self->buffer_, i));
    }
  }
}
...
jint ComGoogleCommonCollectTopKSelector_partitionWithInt_withInt_withInt_(ComGoogleCommonCollectTopKSelector *self, jint left, jint right, jint pivotIndex) {
  ...
  IOSObjectArray_Set(self->buffer_, pivotIndex, IOSObjectArray_Get(self->buffer_, right));
  ...
  IOSObjectArray_Set(self->buffer_, right, IOSObjectArray_Get(self->buffer_, pivotNewIndex));
  IOSObjectArray_Set(self->buffer_, pivotNewIndex, pivotValue);
...
void ComGoogleCommonCollectTopKSelector_swapWithInt_withInt_(ComGoogleCommonCollectTopKSelector *self, jint i, jint j) {
  ...
  IOSObjectArray_Set(self->buffer_, i, IOSObjectArray_Get(self->buffer_, j));
  IOSObjectArray_Set(self->buffer_, j, tmp);
}
