...
//  source: ../target/dependency/com/google/common/primitives/SignedBytes.java
//
...
NSString *ComGoogleCommonPrimitivesSignedBytes_joinWithNSString_withByteArray_(NSString *separator, IOSByteArray *array) {
  ...
  [builder appendWithInt:IOSByteArray_Get(array, 0)];
  for (jint i = 1; i < array->size_; i++) {
    [((JavaLangStringBuilder *) nil_chk([builder appendWithNSString:separator])) appendWithInt:IOSByteArray_Get(array, i)];
  }
...
+ (void)initialize {
  if (self == [ComGoogleCommonPrimitivesSignedBytes_LexicographicalComparator class]) {
    ...
    for (jint i = 0; i < 1; i++) {
      (ComGoogleCommonPrimitivesSignedBytes_LexicographicalComparator_values_[i] = e = objc_constructInstance(self, (void *)ptr), ptr += objSize);
      ComGoogleCommonPrimitivesSignedBytes_LexicographicalComparator_initWithNSString_withInt_(e, names[i], i);
    }
