...
//  source: ../target/dependency/com/google/common/primitives/Ints.java
//
...
IOSByteArray *ComGoogleCommonPrimitivesInts_toByteArrayWithInt_(jint value) {
  ComGoogleCommonPrimitivesInts_initialize();
  return [IOSByteArray arrayWithBytes:(jbyte[]){ (jbyte) (JreRShift32(value, 24)), (jbyte) (JreRShift32(value, 16)), (jbyte) (JreRShift32(value, 8)), (jbyte) value } count:4];
}
...
NSString *ComGoogleCommonPrimitivesInts_joinWithNSString_withIntArray_(NSString *separator, IOSIntArray *array) {
  ...
  [builder appendWithInt:IOSIntArray_Get(array, 0)];
  for (jint i = 1; i < array->size_; i++) {
    [((JavaLangStringBuilder *) nil_chk([builder appendWithNSString:separator])) appendWithInt:IOSIntArray_Get(array, i)];
  }
...
IOSIntArray *ComGoogleCommonPrimitivesInts_toArrayWithJavaUtilCollection_(id<JavaUtilCollection> collection) {
  ...
  IOSIntArray *array = [IOSIntArray arrayWithLength:len];
...
id<JavaUtilList> ComGoogleCommonPrimitivesInts_asListWithIntArray_(IOSIntArray *backingArray) {
  ...
  return create_ComGoogleCommonPrimitivesInts_IntArrayAsList_initWithIntArray_(backingArray);
}
...
+ (void)initialize {
  if (self == [ComGoogleCommonPrimitivesInts_IntConverter class]) {
    JreStrongAssignAndConsume(&ComGoogleCommonPrimitivesInts_IntConverter_INSTANCE, new_ComGoogleCommonPrimitivesInts_IntConverter_init());
...
+ (void)initialize {
  if (self == [ComGoogleCommonPrimitivesInts_LexicographicalComparator class]) {
    ...
    for (jint i = 0; i < 1; i++) {
      (ComGoogleCommonPrimitivesInts_LexicographicalComparator_values_[i] = e = objc_constructInstance(self, (void *)ptr), ptr += objSize);
      ComGoogleCommonPrimitivesInts_LexicographicalComparator_initWithNSString_withInt_(e, names[i], i);
    }
...
- (id<JavaUtilList>)subListWithInt:(jint)fromIndex
  ...
  return create_ComGoogleCommonPrimitivesInts_IntArrayAsList_initWithIntArray_withInt_withInt_(array_, start_ + fromIndex, start_ + toIndex);
}
...
- (NSString *)description {
  JavaLangStringBuilder *builder = create_JavaLangStringBuilder_initWithInt_([self size] * 5);
  [((JavaLangStringBuilder *) nil_chk([builder appendWithChar:'['])) appendWithInt:IOSIntArray_Get(nil_chk(array_), start_)];
  for (jint i = start_ + 1; i < end_; i++) {
    [((JavaLangStringBuilder *) nil_chk([builder appendWithNSString:@", "])) appendWithInt:IOSIntArray_Get(array_, i)];
  }
