...
//  source: ../target/dependency/com/google/common/net/InetAddresses.java
//
...
+ (void)initialize {
  if (self == [ComGoogleCommonNetInetAddresses class]) {
    JreStrongAssign(&ComGoogleCommonNetInetAddresses_IPV4_SPLITTER, [((ComGoogleCommonBaseSplitter *) nil_chk(ComGoogleCommonBaseSplitter_onWithChar_('.'))) limitWithInt:ComGoogleCommonNetInetAddresses_IPV4_PART_COUNT]);
    JreStrongAssign(&ComGoogleCommonNetInetAddresses_LOOPBACK4, (JavaNetInet4Address *) cast_chk(ComGoogleCommonNetInetAddresses_forStringWithNSString_(@"127.0.0.1"), [JavaNetInet4Address class]));
    JreStrongAssign(&ComGoogleCommonNetInetAddresses_ANY4, (JavaNetInet4Address *) cast_chk(ComGoogleCommonNetInetAddresses_forStringWithNSString_(@"0.0.0.0"), [JavaNetInet4Address class]));
...
JavaNetInetAddress *ComGoogleCommonNetInetAddresses_forStringWithNSString_(NSString *ipString) {
  ...
  if (addr == nil) {
    @throw ComGoogleCommonNetInetAddresses_formatIllegalArgumentExceptionWithNSString_withNSObjectArray_(@"'%s' is not an IP string literal.", [IOSObjectArray arrayWithObjects:(id[]){ ipString } count:1 type:NSObject_class_()]);
  }
  return ComGoogleCommonNetInetAddresses_bytesToInetAddressWithByteArray_(addr);
}
...
IOSByteArray *ComGoogleCommonNetInetAddresses_ipStringToBytesWithNSString_(NSString *ipString) {
  ...
  for (jint i = 0; i < ((jint) [((NSString *) nil_chk(ipString)) length]); i++) {
...
IOSByteArray *ComGoogleCommonNetInetAddresses_textToNumericFormatV4WithNSString_(NSString *ipString) {
  ComGoogleCommonNetInetAddresses_initialize();
  IOSByteArray *bytes = [IOSByteArray arrayWithLength:ComGoogleCommonNetInetAddresses_IPV4_PART_COUNT];
  jint i = 0;
  @try {
...
IOSByteArray *ComGoogleCommonNetInetAddresses_textToNumericFormatV6WithNSString_(NSString *ipString) {
  ...
  for (jint i = 1; i < parts->size_ - 1; i++) {
    if (((jint) [((NSString *) nil_chk(IOSObjectArray_Get(parts, i))) length]) == 0) {
      if (skipIndex >= 0) {
        return nil;
      }
      skipIndex = i;
  ...
  if (skipIndex >= 0) {
    ...
    if (((jint) [((NSString *) nil_chk(IOSObjectArray_Get(parts, 0))) length]) == 0 && --partsHi != 0) {
      return nil;
    }
    if (((jint) [((NSString *) nil_chk(IOSObjectArray_Get(parts, parts->size_ - 1))) length]) == 0 && --partsLo != 0) {
      return nil;
    }
  }
  else {
  ...
  @try {
    for (jint i = 0; i < partsHi; i++) {
      [((JavaNioByteBuffer *) nil_chk(rawBytes)) putShortWithShort:ComGoogleCommonNetInetAddresses_parseHextetWithNSString_(IOSObjectArray_Get(parts, i))];
    }
    for (jint i = 0; i < partsSkipped; i++) {
      [((JavaNioByteBuffer *) nil_chk(rawBytes)) putShortWithShort:(jshort) 0];
    }
    for (jint i = partsLo; i > 0; i--) {
      [((JavaNioByteBuffer *) nil_chk(rawBytes)) putShortWithShort:ComGoogleCommonNetInetAddresses_parseHextetWithNSString_(IOSObjectArray_Get(parts, parts->size_ - i))];
    }
  }
  @catch (JavaLangNumberFormatException *ex) {
    return nil;
...
jbyte ComGoogleCommonNetInetAddresses_parseOctetWithNSString_(NSString *ipPart) {
  ...
  if (octet > 255 || ([((NSString *) nil_chk(ipPart)) hasPrefix:@"0"] && ((jint) [ipPart length]) > 1)) {
...
NSString *ComGoogleCommonNetInetAddresses_toAddrStringWithJavaNetInetAddress_(JavaNetInetAddress *ip) {
  ComGoogleCommonNetInetAddresses_initialize();
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(ip);
  ...
  IOSIntArray *hextets = [IOSIntArray arrayWithLength:ComGoogleCommonNetInetAddresses_IPV6_PART_COUNT];
...
NSString *ComGoogleCommonNetInetAddresses_hextetsToIPv6StringWithIntArray_(IOSIntArray *hextets) {
  ComGoogleCommonNetInetAddresses_initialize();
  JavaLangStringBuilder *buf = create_JavaLangStringBuilder_initWithInt_(39);
  ...
  for (jint i = 0; i < ((IOSIntArray *) nil_chk(hextets))->size_; i++) {
    ...
    if (thisIsNumber) {
      ...
      [buf appendWithNSString:JavaLangInteger_toHexStringWithInt_(IOSIntArray_Get(hextets, i))];
    }
    else {
...
JavaNetInetAddress *ComGoogleCommonNetInetAddresses_forUriStringWithNSString_(NSString *hostAddr) {
  ...
  if (addr == nil) {
    @throw ComGoogleCommonNetInetAddresses_formatIllegalArgumentExceptionWithNSString_withNSObjectArray_(@"Not a valid URI IP literal: '%s'", [IOSObjectArray arrayWithObjects:(id[]){ hostAddr } count:1 type:NSObject_class_()]);
  }
  return addr;
}
...
JavaNetInetAddress *ComGoogleCommonNetInetAddresses_forUriStringNoThrowWithNSString_(NSString *hostAddr) {
  ComGoogleCommonNetInetAddresses_initialize();
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(hostAddr);
  NSString *ipString;
  jint expectBytes;
  if ([((NSString *) nil_chk(hostAddr)) hasPrefix:@"["] && [hostAddr hasSuffix:@"]"]) {
    ipString = [hostAddr java_substring:1 endIndex:((jint) [hostAddr length]) - 1];
    expectBytes = 16;
  }
  else {
    ipString = hostAddr;
...
ComGoogleCommonNetInetAddresses_TeredoInfo *ComGoogleCommonNetInetAddresses_getTeredoInfoWithJavaNetInet6Address_(JavaNetInet6Address *ip) {
  ...
  return create_ComGoogleCommonNetInetAddresses_TeredoInfo_initWithJavaNetInet4Address_withJavaNetInet4Address_withInt_withInt_(server, client, port, flags);
}
...
JavaNetInet4Address *ComGoogleCommonNetInetAddresses_getEmbeddedIPv4ClientAddressWithJavaNetInet6Address_(JavaNetInet6Address *ip) {
  ...
  @throw ComGoogleCommonNetInetAddresses_formatIllegalArgumentExceptionWithNSString_withNSObjectArray_(@"'%s' has no embedded IPv4 address.", [IOSObjectArray arrayWithObjects:(id[]){ ComGoogleCommonNetInetAddresses_toAddrStringWithJavaNetInetAddress_(ip) } count:1 type:NSObject_class_()]);
}
...
JavaNetInetAddress *ComGoogleCommonNetInetAddresses_fromLittleEndianByteArrayWithByteArray_(IOSByteArray *addr) {
  ComGoogleCommonNetInetAddresses_initialize();
  IOSByteArray *reversed = [IOSByteArray arrayWithLength:((IOSByteArray *) nil_chk(addr))->size_];
...
JavaLangIllegalArgumentException *ComGoogleCommonNetInetAddresses_formatIllegalArgumentExceptionWithNSString_withNSObjectArray_(NSString *format, IOSObjectArray *args) {
  ComGoogleCommonNetInetAddresses_initialize();
  return create_JavaLangIllegalArgumentException_initWithNSString_(NSString_java_formatWithJavaUtilLocale_withNSString_withNSObjectArray_(JreLoadStatic(JavaUtilLocale, ROOT), format, args));
}
...
void ComGoogleCommonNetInetAddresses_TeredoInfo_initWithJavaNetInet4Address_withJavaNetInet4Address_withInt_withInt_(ComGoogleCommonNetInetAddresses_TeredoInfo *self, JavaNetInet4Address *server, JavaNetInet4Address *client, jint port, jint flags) {
  ...
  JreStrongAssign(&self->server_, ComGoogleCommonBaseMoreObjects_firstNonNullWithId_withId_(server, JreLoadStatic(ComGoogleCommonNetInetAddresses, ANY4)));
  JreStrongAssign(&self->client_, ComGoogleCommonBaseMoreObjects_firstNonNullWithId_withId_(client, JreLoadStatic(ComGoogleCommonNetInetAddresses, ANY4)));
  self->port_ = port;
  self->flags_ = flags;
}
