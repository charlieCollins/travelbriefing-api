...
//  source: ../target/dependency/com/google/common/base/FinalizableReferenceQueue.java
//
...
+ (void)initialize {
  if (self == [ComGoogleCommonBaseFinalizableReferenceQueue class]) {
    JreStrongAssign(&ComGoogleCommonBaseFinalizableReferenceQueue_logger, JavaUtilLoggingLogger_getLoggerWithNSString_([ComGoogleCommonBaseFinalizableReferenceQueue_class_() getName]));
    {
      IOSClass *finalizer = ComGoogleCommonBaseFinalizableReferenceQueue_loadFinalizerWithComGoogleCommonBaseFinalizableReferenceQueue_FinalizerLoaderArray_([IOSObjectArray arrayWithObjects:(id[]){ create_ComGoogleCommonBaseFinalizableReferenceQueue_SystemLoader_init(), create_ComGoogleCommonBaseFinalizableReferenceQueue_DecoupledLoader_init(), create_ComGoogleCommonBaseFinalizableReferenceQueue_DirectLoader_init() } count:3 type:ComGoogleCommonBaseFinalizableReferenceQueue_FinalizerLoader_class_()]);
      JreStrongAssign(&ComGoogleCommonBaseFinalizableReferenceQueue_startFinalizer, ComGoogleCommonBaseFinalizableReferenceQueue_getStartFinalizerWithIOSClass_(finalizer));
    }
...
void ComGoogleCommonBaseFinalizableReferenceQueue_init(ComGoogleCommonBaseFinalizableReferenceQueue *self) {
  ...
  JreStrongAssignAndConsume(&self->frqRef_, new_JavaLangRefPhantomReference_initWithId_withJavaLangRefReferenceQueue_(self, self->queue_));
  jboolean threadStarted = false;
  @try {
    [((JavaLangReflectMethod *) nil_chk(ComGoogleCommonBaseFinalizableReferenceQueue_startFinalizer)) invokeWithId:nil withNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ ComGoogleCommonBaseFinalizableReference_class_(), self->queue_, self->frqRef_ } count:3 type:NSObject_class_()]];
    threadStarted = true;
  }
...
JavaLangReflectMethod *ComGoogleCommonBaseFinalizableReferenceQueue_getStartFinalizerWithIOSClass_(IOSClass *finalizer) {
  ComGoogleCommonBaseFinalizableReferenceQueue_initialize();
  @try {
    return [((IOSClass *) nil_chk(finalizer)) getMethod:@"startFinalizer" parameterTypes:[IOSObjectArray arrayWithObjects:(id[]){ IOSClass_class_(), JavaLangRefReferenceQueue_class_(), JavaLangRefPhantomReference_class_() } count:3 type:IOSClass_class_()]];
  }
...
- (JavaNetURL *)getBaseUrl {
  ...
  if (finalizerUrl == nil) {
    @throw create_JavaIoFileNotFoundException_initWithNSString_(finalizerPath);
  }
  NSString *urlString = [finalizerUrl description];
  if (![((NSString *) nil_chk(urlString)) hasSuffix:finalizerPath]) {
    @throw create_JavaIoIOException_initWithNSString_(JreStrcat("$$", @"Unsupported path style: ", urlString));
  }
  urlString = [urlString java_substring:0 endIndex:((jint) [urlString length]) - ((jint) [finalizerPath length])];
  return create_JavaNetURL_initWithJavaNetURL_withNSString_(finalizerUrl, urlString);
}

- (JavaNetURLClassLoader *)newLoaderWithJavaNetURL:(JavaNetURL *)base {
  return create_JavaNetURLClassLoader_initWithJavaNetURLArray_withJavaLangClassLoader_([IOSObjectArray arrayWithObjects:(id[]){ base } count:1 type:JavaNetURL_class_()], nil);
}
