...
//  source: ../target/dependency/com/google/common/escape/UnicodeEscaper.java
//
...
- (NSString *)escapeWithNSString:(NSString *)string {
  ...
  jint end = ((jint) [((NSString *) nil_chk(string)) length]);
...
NSString *ComGoogleCommonEscapeUnicodeEscaper_escapeSlowWithNSString_withInt_(ComGoogleCommonEscapeUnicodeEscaper *self, NSString *s, jint index) {
  jint end = ((jint) [((NSString *) nil_chk(s)) length]);
  ...
  while (index < end) {
    ...
    if (cp < 0) {
      @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"Trailing high surrogate at end of input");
    }
...
jint ComGoogleCommonEscapeUnicodeEscaper_codePointAtWithJavaLangCharSequence_withInt_withInt_(id<JavaLangCharSequence> seq, jint index, jint end) {
  ...
  if (index < end) {
    ...
    if (c1 < JavaLangCharacter_MIN_HIGH_SURROGATE || c1 > JavaLangCharacter_MAX_LOW_SURROGATE) {
      ...
      @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$C$I$I$@C", @"Expected low surrogate but got char '", c2, @"' with value ", (jint) c2, @" at index ", index, @" in '", seq, '\''));
    }
    else {
      @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$C$I$I$@C", @"Unexpected low surrogate character '", c1, @"' with value ", (jint) c1, @" at index ", (index - 1), @" in '", seq, '\''));
    }
  }
  @throw create_JavaLangIndexOutOfBoundsException_initWithNSString_(@"Index exceeds specified range");
}
...
IOSCharArray *ComGoogleCommonEscapeUnicodeEscaper_growBufferWithCharArray_withInt_withInt_(IOSCharArray *dest, jint index, jint size) {
  ComGoogleCommonEscapeUnicodeEscaper_initialize();
  if (size < 0) {
    @throw create_JavaLangAssertionError_initWithId_(@"Cannot increase internal buffer any further");
  }
  IOSCharArray *copy_ = [IOSCharArray arrayWithLength:size];
  if (index > 0) {
