...
Okhttp3MediaType *Okhttp3MediaType_parseWithNSString_(NSString *string) {
  ...
  for (jint s = [typeSubtype end]; s < ((jint) [((NSString *) nil_chk(string)) length]); s = [parameter end]) {
    [((JavaUtilRegexMatcher *) nil_chk(parameter)) regionWithInt:s withInt:((jint) [string length])];
    ...
    NSString *charsetParameter;
    NSString *token = [parameter groupWithInt:2];
    if (token != nil) {
      charsetParameter = ([token hasPrefix:@"'"] && [token hasSuffix:@"'"] && ((jint) [token length]) > 2) ? [token java_substring:1 endIndex:((jint) [token length]) - 1] : token;
    }
    else {
      charsetParameter = [parameter groupWithInt:3];
    }
    if (charset != nil && ![((NSString *) nil_chk(charsetParameter)) java_equalsIgnoreCase:charset]) {
