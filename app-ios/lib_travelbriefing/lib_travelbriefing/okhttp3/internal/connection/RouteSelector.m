...
//  source: ../target/dependency/okhttp3/internal/connection/RouteSelector.java
//
...
void Okhttp3InternalConnectionRouteSelector_resetNextProxyWithOkhttp3HttpUrl_withJavaNetProxy_(Okhttp3InternalConnectionRouteSelector *self, Okhttp3HttpUrl *url, JavaNetProxy *proxy) {
  if (proxy != nil) {
    ...
    id<JavaUtilList> proxiesOrNull = [((JavaNetProxySelector *) nil_chk([((Okhttp3Address *) nil_chk(self->address_)) proxySelector])) selectWithJavaNetURI:[((Okhttp3HttpUrl *) nil_chk(url)) uri]];
    JreStrongAssign(&self->proxies_, proxiesOrNull != nil && ![proxiesOrNull isEmpty] ? Okhttp3InternalUtil_immutableListWithJavaUtilList_(proxiesOrNull) : Okhttp3InternalUtil_immutableListWithNSObjectArray_([IOSObjectArray arrayWithObjects:(id[]){ JreLoadStatic(JavaNetProxy, NO_PROXY) } count:1 type:JavaNetProxy_class_()]));
  }
