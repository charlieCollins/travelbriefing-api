...
//  source: ../target/dependency/okhttp3/Cookie.java
//
...
- (NSString *)description {
  return [self toStringWithBoolean:false];
}

- (NSString *)toStringWithBoolean:(jboolean)forObsoleteRfc2965 {
  ...
  if (!hostOnly_) {
    [result appendWithNSString:@"; domain="];
    if (forObsoleteRfc2965) {
      [result appendWithNSString:@"."];
    }
    [result appendWithNSString:domain_];
  }
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[22].selector = @selector(toStringWithBoolean:);
  methods[23].selector = @selector(isEqual:);
  methods[24].selector = @selector(hash);
  #pragma clang diagnostic pop
  static const J2ObjcFieldInfo fields[] = {
    { "YEAR_PATTERN", "LJavaUtilRegexPattern;", .constantValue.asLong = 0, 0x1a, -1, 24, -1, -1 },
    { "MONTH_PATTERN", "LJavaUtilRegexPattern;", .constantValue.asLong = 0, 0x1a, -1, 25, -1, -1 },
    { "DAY_OF_MONTH_PATTERN", "LJavaUtilRegexPattern;", .constantValue.asLong = 0, 0x1a, -1, 26, -1, -1 },
    { "TIME_PATTERN", "LJavaUtilRegexPattern;", .constantValue.asLong = 0, 0x1a, -1, 27, -1, -1 },
...
jboolean Okhttp3Cookie_domainMatchWithOkhttp3HttpUrl_withNSString_(Okhttp3HttpUrl *url, NSString *domain) {
  ...
  if ([urlHost hasSuffix:domain] && [urlHost charAtWithInt:((jint) [urlHost length]) - ((jint) [((NSString *) nil_chk(domain)) length]) - 1] == '.' && !Okhttp3InternalUtil_verifyAsIpAddressWithNSString_(urlHost)) {
    return true;
  }
  return false;
}
...
jboolean Okhttp3Cookie_pathMatchWithOkhttp3HttpUrl_withNSString_(Okhttp3HttpUrl *url, NSString *path) {
  ...
  if ([urlPath hasPrefix:path]) {
    if ([((NSString *) nil_chk(path)) hasSuffix:@"/"]) return true;
    if ([urlPath charAtWithInt:((jint) [path length])] == '/') return true;
  }
  return false;
}
...
Okhttp3Cookie *Okhttp3Cookie_parseWithLong_withOkhttp3HttpUrl_withNSString_(jlong currentTimeMillis, Okhttp3HttpUrl *url, NSString *setCookie) {
  Okhttp3Cookie_initialize();
  jint pos = 0;
  jint limit = ((jint) [((NSString *) nil_chk(setCookie)) length]);
  ...
  if ([((NSString *) nil_chk(cookieName)) java_isEmpty] || Okhttp3InternalUtil_indexOfControlOrNonAsciiWithNSString_(cookieName) != -1) return nil;
  ...
  if (Okhttp3InternalUtil_indexOfControlOrNonAsciiWithNSString_(cookieValue) != -1) return nil;
  ...
  while (pos < limit) {
    ...
    if ([((NSString *) nil_chk(attributeName)) java_equalsIgnoreCase:@"expires"]) {
      @try {
        expiresAt = Okhttp3Cookie_parseExpiresWithNSString_withInt_withInt_(attributeValue, 0, ((jint) [attributeValue length]));
        persistent = true;
      }
...
jlong Okhttp3Cookie_parseMaxAgeWithNSString_(NSString *s) {
  ...
  @catch (JavaLangNumberFormatException *e) {
    if ([((NSString *) nil_chk(s)) java_matches:@"-?\\d+"]) {
      return [s hasPrefix:@"-"] ? JavaLangLong_MIN_VALUE : JavaLangLong_MAX_VALUE;
    }
    @throw e;
  }
}
...
- (Okhttp3Cookie_Builder *)pathWithNSString:(NSString *)path {
  if (![((NSString *) nil_chk(path)) hasPrefix:@"/"]) @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"path must start with '/'");
  JreStrongAssign(&self->path_, path);
  return self;
}
