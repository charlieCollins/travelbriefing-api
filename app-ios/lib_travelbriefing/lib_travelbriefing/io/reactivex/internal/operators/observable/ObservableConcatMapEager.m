...
#include "io/reactivex/internal/observers/InnerQueuedObserver.h"
      ...
      while (ac != maxConcurrency_) {
        ...
        if (errorMode == JreLoadEnum(IoReactivexInternalUtilErrorMode, IMMEDIATE)) {
          ...
          if (ex != nil) {
            [((id<IoReactivexInternalFuseableSimpleQueue>) nil_chk(q)) clear];
            [self disposeAll];
            [((id<IoReactivexObserver>) nil_chk(a)) onErrorWithNSException:[error_ terminate]];
            return;
          }
        }
        id v;
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[9].selector = @selector(innerErrorWithIoReactivexInternalObserversInnerQueuedObserver:withNSException:);
