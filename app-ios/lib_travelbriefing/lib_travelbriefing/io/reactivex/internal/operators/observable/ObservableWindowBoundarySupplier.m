...
- (void)next {
  [((id<IoReactivexInternalFuseableSimpleQueue>) nil_chk(queue_)) offerWithId:IoReactivexInternalOperatorsObservableObservableWindowBoundarySupplier_WindowBoundaryMainObserver_NEXT];
  if ([self enter]) {
    [self drainLoop];
  }
}
...
- (void)onErrorWithNSException:(NSException *)t {
  ...
  done_ = true;
  [((IoReactivexInternalOperatorsObservableObservableWindowBoundarySupplier_WindowBoundaryMainObserver *) nil_chk(parent_)) onErrorWithNSException:t];
}
