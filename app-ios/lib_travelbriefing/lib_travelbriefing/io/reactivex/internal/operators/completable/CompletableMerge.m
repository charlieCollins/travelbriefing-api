  ...
  if (!delayErrors_) {
    ...
    if ([((IoReactivexInternalUtilAtomicThrowable *) nil_chk(error_)) addThrowableWithNSException:t]) {
      if ([self getAndSetWithInt:0] > 0) {
        [((id<IoReactivexCompletableObserver>) nil_chk(actual_)) onErrorWithNSException:[error_ terminate]];
      }
    }
    else {
...
- (void)onComplete {
  if ([self decrementAndGet] == 0) {
    NSException *ex = [((IoReactivexInternalUtilAtomicThrowable *) nil_chk(error_)) get];
    if (ex != nil) {
      [((id<IoReactivexCompletableObserver>) nil_chk(actual_)) onErrorWithNSException:[error_ terminate]];
    }
    else {
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[7].selector = @selector(innerErrorWithIoReactivexInternalOperatorsCompletableCompletableMerge_CompletableMergeSubscriber_MergeInnerObserver:withNSException:);
  ...
  static const void *ptrTable[] = { "LIoReactivexCompletableObserver;IZ", "onSubscribe", "LOrgReactivestreamsSubscription;", "onNext", "LIoReactivexCompletableSource;", "onError", "LNSException;", "innerError", "LIoReactivexInternalOperatorsCompletableCompletableMerge_CompletableMergeSubscriber_MergeInnerObserver;LNSException;", "innerComplete", "LIoReactivexInternalOperatorsCompletableCompletableMerge_CompletableMergeSubscriber_MergeInnerObserver;", "LIoReactivexInternalOperatorsCompletableCompletableMerge;", "Ljava/util/concurrent/atomic/AtomicInteger;Lorg/reactivestreams/Subscriber<Lio/reactivex/CompletableSource;>;Lio/reactivex/disposables/Disposable;" };
...
- (void)onErrorWithNSException:(NSException *)e {
  [this$0_ innerErrorWithIoReactivexInternalOperatorsCompletableCompletableMerge_CompletableMergeSubscriber_MergeInnerObserver:self withNSException:e];
}
