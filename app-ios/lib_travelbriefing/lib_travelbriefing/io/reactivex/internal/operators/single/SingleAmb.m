...
//  source: ../target/dependency/io/reactivex/internal/operators/single/SingleAmb.java
//
...
- (void)subscribeActualWithIoReactivexSingleObserver:(id<IoReactivexSingleObserver>)s {
  ...
  if (sources == nil) {
    ...
    @try {
      for (id<IoReactivexSingleSource> __strong element in nil_chk(sourcesIterable_)) {
        if (element == nil) {
          IoReactivexInternalDisposablesEmptyDisposable_errorWithNSException_withIoReactivexSingleObserver_(create_JavaLangNullPointerException_initWithNSString_(@"One of the sources is null"), s);
          return;
        }
  ...
  for (jint i = 0; i < count; i++) {
    ...
    if (s1 == nil) {
      [set dispose];
      NSException *e = create_JavaLangNullPointerException_initWithNSString_(@"One of the sources is null");
...
- (void)onErrorWithNSException:(NSException *)e {
  if ([self compareAndSetWithBoolean:false withBoolean:true]) {
    [((IoReactivexDisposablesCompositeDisposable *) nil_chk(set_)) dispose];
    [((id<IoReactivexSingleObserver>) nil_chk(s_)) onErrorWithNSException:e];
  }
  else {
    IoReactivexPluginsRxJavaPlugins_onErrorWithNSException_(e);
  }
