...
//  source: ../target/dependency/com/google/common/hash/AbstractNonStreamingHashFunction.java
//
...
- (id<ComGoogleCommonHashHasher>)newHasher {
  return create_ComGoogleCommonHashAbstractNonStreamingHashFunction_BufferingHasher_initWithComGoogleCommonHashAbstractNonStreamingHashFunction_withInt_(self, 32);
}
...
- (id<ComGoogleCommonHashHasher>)newHasherWithInt:(jint)expectedInputSize {
  ...
  return create_ComGoogleCommonHashAbstractNonStreamingHashFunction_BufferingHasher_initWithComGoogleCommonHashAbstractNonStreamingHashFunction_withInt_(self, expectedInputSize);
}
...
- (ComGoogleCommonHashHashCode *)hashUnencodedCharsWithJavaLangCharSequence:(id<JavaLangCharSequence>)input {
  jint len = [((id<JavaLangCharSequence>) nil_chk(input)) length];
  id<ComGoogleCommonHashHasher> hasher = [self newHasherWithInt:len * 2];
  for (jint i = 0; i < len; i++) {
    [((id<ComGoogleCommonHashHasher>) nil_chk(hasher)) putCharWithChar:[input charAtWithInt:i]];
  }
...
void ComGoogleCommonHashAbstractNonStreamingHashFunction_BufferingHasher_initWithComGoogleCommonHashAbstractNonStreamingHashFunction_withInt_(ComGoogleCommonHashAbstractNonStreamingHashFunction_BufferingHasher *self, ComGoogleCommonHashAbstractNonStreamingHashFunction *outer$, jint expectedInputSize) {
  ...
  JreStrongAssignAndConsume(&self->stream_, new_ComGoogleCommonHashAbstractNonStreamingHashFunction_ExposedByteArrayOutputStream_initWithInt_(expectedInputSize));
}
