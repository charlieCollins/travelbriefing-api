...
//  source: ../target/dependency/com/google/common/reflect/TypeVisitor.java
//
    ...
    while (b__ < e__) {
      ...
      @try {
        if ([JavaLangReflectTypeVariable_class_() isInstance:type]) {
          ...
          @throw create_JavaLangAssertionError_initWithId_(JreStrcat("$@", @"Unknown type: ", type));
        }
        succeeded = true;
      }
      @finally {
...
void ComGoogleCommonReflectTypeVisitor_init(ComGoogleCommonReflectTypeVisitor *self) {
  NSObject_init(self);
  JreStrongAssign(&self->visited_, ComGoogleCommonCollectSets_newHashSet());
}
