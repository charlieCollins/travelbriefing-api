...
//  source: ../target/dependency/com/google/common/util/concurrent/AtomicDoubleArray.java
//
...
- (NSString *)description {
  ...
  JavaLangStringBuilder *b = create_JavaLangStringBuilder_initWithInt_((17 + 2) * (iMax + 1));
  [b appendWithChar:'['];
  for (jint i = 0; ; i++) {
    [b appendWithDouble:JavaLangDouble_longBitsToDoubleWithLong_([((JavaUtilConcurrentAtomicAtomicLongArray *) nil_chk(longs_)) getWithInt:i])];
    if (i == iMax) {
...
- (void)readObjectWithJavaIoObjectInputStream:(JavaIoObjectInputStream *)s {
  ...
  JreStrongAssignAndConsume(&self->longs_, new_JavaUtilConcurrentAtomicAtomicLongArray_initWithInt_(length));
  for (jint i = 0; i < length; i++) {
...
void ComGoogleCommonUtilConcurrentAtomicDoubleArray_initWithDoubleArray_(ComGoogleCommonUtilConcurrentAtomicDoubleArray *self, IOSDoubleArray *array) {
  ...
  IOSLongArray *longArray = [IOSLongArray arrayWithLength:len];
  for (jint i = 0; i < len; i++) {
    *IOSLongArray_GetRef(longArray, i) = JavaLangDouble_doubleToRawLongBitsWithDouble_(IOSDoubleArray_Get(array, i));
  }
  JreStrongAssignAndConsume(&self->longs_, new_JavaUtilConcurrentAtomicAtomicLongArray_initWithLongArray_(longArray));
}
