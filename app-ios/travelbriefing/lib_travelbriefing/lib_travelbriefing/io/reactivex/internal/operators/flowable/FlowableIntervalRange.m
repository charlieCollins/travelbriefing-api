...
//  source: ../target/dependency/io/reactivex/internal/operators/flowable/FlowableIntervalRange.java
//
...
- (void)run {
  if ([((JavaUtilConcurrentAtomicAtomicReference *) nil_chk(resource_)) get] != JreLoadEnum(IoReactivexInternalDisposablesDisposableHelper, DISPOSED)) {
    ...
    if (r != 0LL) {
      ...
      [((id<OrgReactivestreamsSubscriber>) nil_chk(actual_)) onErrorWithNSException:create_IoReactivexExceptionsMissingBackpressureException_initWithNSString_(JreStrcat("$J$", @"Can't deliver value ", count_, @" due to lack of requests"))];
      IoReactivexInternalDisposablesDisposableHelper_disposeWithJavaUtilConcurrentAtomicAtomicReference_(resource_);
    }
  }
}
