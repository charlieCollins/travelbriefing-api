...
//  source: ../target/dependency/io/reactivex/internal/operators/observable/ObservableFromArray.java
//
...
- (void)run {
  ...
  for (jint i = 0; i < n && ![self isDisposed]; i++) {
    ...
    if (value == nil) {
      [((id<IoReactivexObserver>) nil_chk(actual_)) onErrorWithNSException:create_JavaLangNullPointerException_initWithNSString_(JreStrcat("$I$", @"The ", i, @"th element is null"))];
      return;
    }
