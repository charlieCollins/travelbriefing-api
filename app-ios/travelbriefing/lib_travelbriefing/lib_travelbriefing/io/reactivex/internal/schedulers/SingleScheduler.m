...
//  source: ../target/dependency/io/reactivex/internal/schedulers/SingleScheduler.java
//
...
- (id<IoReactivexDisposablesDisposable>)scheduleDirectWithJavaLangRunnable:(id<JavaLangRunnable>)run
  ...
  @try {
    ...
    if (delay <= 0LL) {
      f = [((id<JavaUtilConcurrentScheduledExecutorService>) nil_chk([((JavaUtilConcurrentAtomicAtomicReference *) nil_chk(executor_)) get])) submitWithJavaLangRunnable:decoratedRun];
    }
    else {
      f = [((id<JavaUtilConcurrentScheduledExecutorService>) nil_chk([((JavaUtilConcurrentAtomicAtomicReference *) nil_chk(executor_)) get])) scheduleWithJavaLangRunnable:decoratedRun withLong:delay withJavaUtilConcurrentTimeUnit:unit];
    }
...
- (id<IoReactivexDisposablesDisposable>)schedulePeriodicallyDirectWithJavaLangRunnable:(id<JavaLangRunnable>)run
  ...
  id<JavaLangRunnable> decoratedRun = IoReactivexPluginsRxJavaPlugins_onScheduleWithJavaLangRunnable_(run);
  @try {
    id<JavaUtilConcurrentFuture> f = [((id<JavaUtilConcurrentScheduledExecutorService>) nil_chk([((JavaUtilConcurrentAtomicAtomicReference *) nil_chk(executor_)) get])) scheduleAtFixedRateWithJavaLangRunnable:decoratedRun withLong:initialDelay withLong:period withJavaUtilConcurrentTimeUnit:unit];
    return IoReactivexDisposablesDisposables_fromFutureWithJavaUtilConcurrentFuture_(f);
  }
  @catch (JavaUtilConcurrentRejectedExecutionException *ex) {
    IoReactivexPluginsRxJavaPlugins_onErrorWithNSException_(ex);
