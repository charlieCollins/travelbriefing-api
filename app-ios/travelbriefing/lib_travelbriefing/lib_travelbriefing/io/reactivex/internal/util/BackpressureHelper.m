...
//  source: ../target/dependency/io/reactivex/internal/util/BackpressureHelper.java
//
...
jlong IoReactivexInternalUtilBackpressureHelper_producedWithJavaUtilConcurrentAtomicAtomicLong_withLong_(JavaUtilConcurrentAtomicAtomicLong *requested, jlong n) {
  ...
  for (; ; ) {
    ...
    if (update < 0LL) {
      IoReactivexPluginsRxJavaPlugins_onErrorWithNSException_(create_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$J", @"More produced than requested: ", update)));
      update = 0LL;
    }
