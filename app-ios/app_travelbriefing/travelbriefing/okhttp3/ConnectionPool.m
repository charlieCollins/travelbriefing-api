...
//  source: ../target/dependency/okhttp3/ConnectionPool.java
//
...
- (Okhttp3InternalConnectionRealConnection *)getWithOkhttp3Address:(Okhttp3Address *)address
  ...
  for (Okhttp3InternalConnectionRealConnection * __strong connection in nil_chk(connections_)) {
    if ([((Okhttp3InternalConnectionRealConnection *) nil_chk(connection)) isEligibleWithOkhttp3Address:address]) {
...
- (JavaNetSocket *)deduplicateWithOkhttp3Address:(Okhttp3Address *)address
   withOkhttp3InternalConnectionStreamAllocation:(Okhttp3InternalConnectionStreamAllocation *)streamAllocation {
  JreAssert(((JavaLangThread_holdsLockWithId_(self))), (@"okhttp3/ConnectionPool.java:134 condition failed: assert (Thread.holdsLock(this));"));
  for (Okhttp3InternalConnectionRealConnection * __strong connection in nil_chk(connections_)) {
    if ([((Okhttp3InternalConnectionRealConnection *) nil_chk(connection)) isEligibleWithOkhttp3Address:address] && [connection isMultiplexed] && connection != [((Okhttp3InternalConnectionStreamAllocation *) nil_chk(streamAllocation)) connection]) {
      return [((Okhttp3InternalConnectionStreamAllocation *) nil_chk(streamAllocation)) releaseAndAcquireWithOkhttp3InternalConnectionRealConnection:connection];
    }
  }
  return nil;
}
...
- (void)putWithOkhttp3InternalConnectionRealConnection:(Okhttp3InternalConnectionRealConnection *)connection {
  JreAssert(((JavaLangThread_holdsLockWithId_(self))), (@"okhttp3/ConnectionPool.java:146 condition failed: assert (Thread.holdsLock(this));"));
  if (!cleanupRunning_) {
...
- (jboolean)connectionBecameIdleWithOkhttp3InternalConnectionRealConnection:(Okhttp3InternalConnectionRealConnection *)connection {
  JreAssert(((JavaLangThread_holdsLockWithId_(self))), (@"okhttp3/ConnectionPool.java:159 condition failed: assert (Thread.holdsLock(this));"));
...
+ (const J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { NULL, "LJavaNetSocket;", 0x0, 3, 2, -1, -1, -1, -1 },
  ...
  methods[5].selector = @selector(deduplicateWithOkhttp3Address:withOkhttp3InternalConnectionStreamAllocation:);
  methods[6].selector = @selector(putWithOkhttp3InternalConnectionRealConnection:);
  methods[7].selector = @selector(connectionBecameIdleWithOkhttp3InternalConnectionRealConnection:);
  methods[8].selector = @selector(evictAll);
  methods[9].selector = @selector(cleanupWithLong:);
  methods[10].selector = @selector(pruneAndGetAllocationCountWithOkhttp3InternalConnectionRealConnection:withLong:);
  #pragma clang diagnostic pop
  static const J2ObjcFieldInfo fields[] = {
    { "executor", "LJavaUtilConcurrentExecutor;", .constantValue.asLong = 0, 0x1a, -1, 11, -1, -1 },
    ...
    { "connections_", "LJavaUtilDeque;", .constantValue.asLong = 0, 0x12, -1, -1, 12, -1 },
  ...
  static const void *ptrTable[] = { "IJLJavaUtilConcurrentTimeUnit;", "get", "LOkhttp3Address;LOkhttp3InternalConnectionStreamAllocation;", "deduplicate", "put", "LOkhttp3InternalConnectionRealConnection;", "connectionBecameIdle", "cleanup", "J", "pruneAndGetAllocationCount", "LOkhttp3InternalConnectionRealConnection;J", &Okhttp3ConnectionPool_executor, "Ljava/util/Deque<Lokhttp3/internal/connection/RealConnection;>;" };
  static const J2ObjcClassInfo _Okhttp3ConnectionPool = { "ConnectionPool", "okhttp3", ptrTable, methods, fields, 7, 0x11, 11, 7, -1, -1, -1, -1, -1 };
...
jint Okhttp3ConnectionPool_pruneAndGetAllocationCountWithOkhttp3InternalConnectionRealConnection_withLong_(Okhttp3ConnectionPool *self, Okhttp3InternalConnectionRealConnection *connection, jlong now) {
  ...
  for (jint i = 0; i < [((id<JavaUtilList>) nil_chk(references)) size]; ) {
    ...
    Okhttp3InternalConnectionStreamAllocation_StreamAllocationReference *streamAllocRef = (Okhttp3InternalConnectionStreamAllocation_StreamAllocationReference *) cast_chk(reference, [Okhttp3InternalConnectionStreamAllocation_StreamAllocationReference class]);
    NSString *message = JreStrcat("$@$", @"A connection to ", [((Okhttp3Address *) nil_chk([((Okhttp3Route *) nil_chk([connection route])) address])) url], @" was leaked. Did you forget to close a response body?");
    [((Okhttp3InternalPlatformPlatform *) nil_chk(Okhttp3InternalPlatformPlatform_get())) logCloseableLeakWithNSString:message withId:streamAllocRef->callStackTrace_];
