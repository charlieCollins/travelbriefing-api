...
Okhttp3CacheControl *Okhttp3CacheControl_parseWithOkhttp3Headers_(Okhttp3Headers *headers) {
  ...
  for (jint i = 0, size = [((Okhttp3Headers *) nil_chk(headers)) size]; i < size; i++) {
    ...
    while (pos < ((jint) [((NSString *) nil_chk(value)) length])) {
      ...
      NSString *parameter;
      if (pos == ((jint) [value length]) || [value charAtWithInt:pos] == ',' || [value charAtWithInt:pos] == ';') {
        pos++;
        parameter = nil;
      }
      else {
        ...
        if (pos < ((jint) [value length]) && [value charAtWithInt:pos] == '"') {
          pos++;
...
NSString *Okhttp3CacheControl_headerValue(Okhttp3CacheControl *self) {
  ...
  [result delete__WithInt:[result length] - 2 withInt:[result length]];
