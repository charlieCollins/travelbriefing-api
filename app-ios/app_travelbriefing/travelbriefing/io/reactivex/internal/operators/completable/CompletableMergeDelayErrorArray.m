...
- (void)subscribeActualWithIoReactivexCompletableObserver:(id<IoReactivexCompletableObserver>)s {
  ...
  [((id<IoReactivexCompletableObserver>) nil_chk(s)) onSubscribeWithIoReactivexDisposablesDisposable:set];
  {
    ...
    while (b__ < e__) {
      ...
      if (c == nil) {
        NSException *ex = create_JavaLangNullPointerException_initWithNSString_(@"A completable source is null");
        [error addThrowableWithNSException:ex];
        [wip decrementAndGet];
        continue;
      }
...
- (void)tryTerminate {
  if ([((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(wip_)) decrementAndGet] == 0) {
    NSException *ex = [((IoReactivexInternalUtilAtomicThrowable *) nil_chk(error_)) terminate];
    if (ex == nil) {
