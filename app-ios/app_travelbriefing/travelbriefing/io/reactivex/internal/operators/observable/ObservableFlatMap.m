...
//  source: ../target/dependency/io/reactivex/internal/operators/observable/ObservableFlatMap.java
//
...
- (void)subscribeInnerWithIoReactivexObservableSource:(id<IoReactivexObservableSource>)p {
  for (; ; ) {
    if ([JavaUtilConcurrentCallable_class_() isInstance:p]) {
      ...
      if (maxConcurrency_ != JavaLangInteger_MAX_VALUE) {
        @synchronized(self) {
          p = [((id<JavaUtilQueue>) nil_chk(sources_)) poll];
          if (p == nil) {
            wip_--;
            break;
          }
  ...
  for (; ; ) {
    ...
    if (d && (svq == nil || [svq isEmpty]) && n == 0) {
      ...
      if (ex == nil) {
        ...
        [((id<IoReactivexObserver>) nil_chk(child)) onErrorWithNSException:[errors_ terminate]];
      }
      return;
    }
...
- (jboolean)checkTerminate {
  ...
  if (!delayErrors_ && (e != nil)) {
    [self disposeAll];
    [((id<IoReactivexObserver>) nil_chk(actual_)) onErrorWithNSException:[errors_ terminate]];
    return true;
  }
  return false;
}
...
- (void)onErrorWithNSException:(NSException *)t {
  if ([((IoReactivexInternalUtilAtomicThrowable *) nil_chk(((IoReactivexInternalOperatorsObservableObservableFlatMap_MergeObserver *) nil_chk(parent_))->errors_)) addThrowableWithNSException:t]) {
