...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  methods[0].selector = @selector(initWithOkioSink:withJavaUtilZipDeflater:);
  methods[1].selector = @selector(initWithOkioBufferedSink:withJavaUtilZipDeflater:);
  methods[2].selector = @selector(writeWithOkioBuffer:withLong:);
  methods[3].selector = @selector(deflateWithBoolean:);
  methods[4].selector = @selector(flush);
  methods[5].selector = @selector(finishDeflate);
  methods[6].selector = @selector(close);
  methods[7].selector = @selector(timeout);
...
void OkioDeflaterSink_initWithOkioBufferedSink_withJavaUtilZipDeflater_(OkioDeflaterSink *self, id<OkioBufferedSink> sink, JavaUtilZipDeflater *deflater) {
  NSObject_init(self);
  if (sink == nil) @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"source == null");
  if (deflater == nil) @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"inflater == null");
  JreStrongAssign(&self->sink_, sink);
  JreStrongAssign(&self->deflater_, deflater);
}
...
void OkioDeflaterSink_deflateWithBoolean_(OkioDeflaterSink *self, jboolean syncFlush) {
  ...
  while (true) {
    ...
    if (deflated > 0) {
      ...
      [self->sink_ emitCompleteSegments];
    }
    else if ([self->deflater_ needsInput]) {
      if (s->pos_ == s->limit_) {
        JreStrongAssign(&buffer->head_, [s pop]);
