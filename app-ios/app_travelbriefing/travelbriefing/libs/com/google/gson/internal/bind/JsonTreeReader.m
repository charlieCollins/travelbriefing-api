...
//  source: ../target/dependency/com/google/gson/internal/bind/JsonTreeReader.java
//
...
- (NSString *)nextName {
  ...
  IOSObjectArray_Set(nil_chk(pathNames_JsonTreeReader_), stackSize_JsonTreeReader_ - 1, result);
...
- (NSString *)nextString {
  ...
  if (token != JreLoadEnum(ComGoogleGsonStreamJsonToken, STRING) && token != JreLoadEnum(ComGoogleGsonStreamJsonToken, NUMBER)) {
    @throw create_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$@$@$", @"Expected ", JreLoadEnum(ComGoogleGsonStreamJsonToken, STRING), @" but was ", token, ComGoogleGsonInternalBindJsonTreeReader_locationString(self)));
  }
...
- (jdouble)nextDouble {
  ...
  if (token != JreLoadEnum(ComGoogleGsonStreamJsonToken, NUMBER) && token != JreLoadEnum(ComGoogleGsonStreamJsonToken, STRING)) {
    @throw create_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$@$@$", @"Expected ", JreLoadEnum(ComGoogleGsonStreamJsonToken, NUMBER), @" but was ", token, ComGoogleGsonInternalBindJsonTreeReader_locationString(self)));
  }
...
- (void)close {
  JreStrongAssignAndConsume(&stack_JsonTreeReader_, [IOSObjectArray newArrayWithObjects:(id[]){ ComGoogleGsonInternalBindJsonTreeReader_SENTINEL_CLOSED } count:1 type:NSObject_class_()]);
...
- (void)skipValue {
  if ([self peek] == JreLoadEnum(ComGoogleGsonStreamJsonToken, NAME)) {
    [self nextName];
    IOSObjectArray_Set(nil_chk(pathNames_JsonTreeReader_), stackSize_JsonTreeReader_ - 2, @"null");
  }
  else {
    ComGoogleGsonInternalBindJsonTreeReader_popStack(self);
    IOSObjectArray_Set(nil_chk(pathNames_JsonTreeReader_), stackSize_JsonTreeReader_ - 1, @"null");
  }
  (*IOSIntArray_GetRef(nil_chk(pathIndices_JsonTreeReader_), stackSize_JsonTreeReader_ - 1))++;
}
...
- (void)promoteNameToValue {
  ...
  ComGoogleGsonInternalBindJsonTreeReader_pushWithId_(self, create_ComGoogleGsonJsonPrimitive_initWithNSString_((NSString *) cast_chk([entry_ getKey], [NSString class])));
}
  ...
  for (jint i = 0; i < stackSize_JsonTreeReader_; i++) {
    if ([IOSObjectArray_Get(nil_chk(stack_JsonTreeReader_), i) isKindOfClass:[ComGoogleGsonJsonArray class]]) {
      if ([JavaUtilIterator_class_() isInstance:IOSObjectArray_Get(stack_JsonTreeReader_, ++i)]) {
        [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk(result)) appendWithChar:'['])) appendWithInt:IOSIntArray_Get(nil_chk(pathIndices_JsonTreeReader_), i)])) appendWithChar:']'];
      }
    }
      ...
      if ([JavaUtilIterator_class_() isInstance:IOSObjectArray_Get(stack_JsonTreeReader_, ++i)]) {
        ...
        if (IOSObjectArray_Get(nil_chk(pathNames_JsonTreeReader_), i) != nil) {
          [result appendWithNSString:IOSObjectArray_Get(pathNames_JsonTreeReader_, i)];
        }
      }
    }
  }
...
- (void)dealloc {
  ...
  RELEASE_(pathNames_JsonTreeReader_);
  RELEASE_(pathIndices_JsonTreeReader_);
  [super dealloc];
}
...
void ComGoogleGsonInternalBindJsonTreeReader_initWithComGoogleGsonJsonElement_(ComGoogleGsonInternalBindJsonTreeReader *self, ComGoogleGsonJsonElement *element) {
  ...
  JreStrongAssignAndConsume(&self->stack_JsonTreeReader_, [IOSObjectArray newArrayWithLength:32 type:NSObject_class_()]);
  self->stackSize_JsonTreeReader_ = 0;
  JreStrongAssignAndConsume(&self->pathNames_JsonTreeReader_, [IOSObjectArray newArrayWithLength:32 type:NSString_class_()]);
  JreStrongAssignAndConsume(&self->pathIndices_JsonTreeReader_, [IOSIntArray newArrayWithLength:32]);
...
id ComGoogleGsonInternalBindJsonTreeReader_popStack(ComGoogleGsonInternalBindJsonTreeReader *self) {
  ...
  IOSObjectArray_Set(self->stack_JsonTreeReader_, self->stackSize_JsonTreeReader_, nil);
  return result;
}
...
void ComGoogleGsonInternalBindJsonTreeReader_expectWithComGoogleGsonStreamJsonToken_(ComGoogleGsonInternalBindJsonTreeReader *self, ComGoogleGsonStreamJsonToken *expected) {
  if ([self peek] != expected) {
    @throw create_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$@$@$", @"Expected ", expected, @" but was ", [self peek], ComGoogleGsonInternalBindJsonTreeReader_locationString(self)));
  }
}
...
void ComGoogleGsonInternalBindJsonTreeReader_pushWithId_(ComGoogleGsonInternalBindJsonTreeReader *self, id newTop) {
  if (self->stackSize_JsonTreeReader_ == ((IOSObjectArray *) nil_chk(self->stack_JsonTreeReader_))->size_) {
    IOSObjectArray *newStack = [IOSObjectArray arrayWithLength:self->stackSize_JsonTreeReader_ * 2 type:NSObject_class_()];
    IOSIntArray *newPathIndices = [IOSIntArray arrayWithLength:self->stackSize_JsonTreeReader_ * 2];
    IOSObjectArray *newPathNames = [IOSObjectArray arrayWithLength:self->stackSize_JsonTreeReader_ * 2 type:NSString_class_()];
    ...
    JreStrongAssign(&self->stack_JsonTreeReader_, newStack);
    JreStrongAssign(&self->pathIndices_JsonTreeReader_, newPathIndices);
    JreStrongAssign(&self->pathNames_JsonTreeReader_, newPathNames);
  }
  IOSObjectArray_Set(self->stack_JsonTreeReader_, self->stackSize_JsonTreeReader_++, newTop);
}
