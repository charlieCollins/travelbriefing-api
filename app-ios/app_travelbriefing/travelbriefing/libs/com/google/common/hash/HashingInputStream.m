...
- (jint)read {
  ...
  if (b != -1) {
    [((id<ComGoogleCommonHashHasher>) nil_chk(hasher_)) putByteWithByte:(jbyte) b];
  }
  return b;
}
...
- (jint)readWithByteArray:(IOSByteArray *)bytes
  ...
  if (numOfBytesRead != -1) {
    [((id<ComGoogleCommonHashHasher>) nil_chk(hasher_)) putBytesWithByteArray:bytes withInt:off withInt:numOfBytesRead];
  }
...
- (void)reset {
  @throw create_JavaIoIOException_initWithNSString_(@"reset not supported");
}
...
void ComGoogleCommonHashHashingInputStream_initWithComGoogleCommonHashHashFunction_withJavaIoInputStream_(ComGoogleCommonHashHashingInputStream *self, id<ComGoogleCommonHashHashFunction> hashFunction, JavaIoInputStream *inArg) {
  ...
  JreStrongAssign(&self->hasher_, ComGoogleCommonBasePreconditions_checkNotNullWithId_([((id<ComGoogleCommonHashHashFunction>) nil_chk(hashFunction)) newHasher]));
}
