...
- (NSString *)escapeWithNSString:(NSString *)s {
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(s);
...
- (IOSCharArray *)escapeWithInt:(jint)cp {
  if (cp < ((IOSBooleanArray *) nil_chk(safeOctets_))->size_ && IOSBooleanArray_Get(safeOctets_, cp)) {
    ...
    IOSCharArray *dest = [IOSCharArray arrayWithLength:3];
    ...
    *IOSCharArray_GetRef(dest, 1) = IOSCharArray_Get(ComGoogleCommonNetPercentEscaper_UPPER_HEX_DIGITS, JreURShift32(cp, 4));
    ...
    IOSCharArray *dest = [IOSCharArray arrayWithLength:6];
    *IOSCharArray_GetRef(dest, 0) = '%';
    ...
    *IOSCharArray_GetRef(dest, 1) = IOSCharArray_Get(ComGoogleCommonNetPercentEscaper_UPPER_HEX_DIGITS, (jint) 0xC | cp);
    ...
    IOSCharArray *dest = [IOSCharArray arrayWithLength:9];
    *IOSCharArray_GetRef(dest, 0) = '%';
    ...
    *IOSCharArray_GetRef(dest, 2) = IOSCharArray_Get(ComGoogleCommonNetPercentEscaper_UPPER_HEX_DIGITS, cp);
    ...
    IOSCharArray *dest = [IOSCharArray arrayWithLength:12];
    *IOSCharArray_GetRef(dest, 0) = '%';
    ...
    *IOSCharArray_GetRef(dest, 2) = IOSCharArray_Get(ComGoogleCommonNetPercentEscaper_UPPER_HEX_DIGITS, cp & (jint) 0x7);
    ...
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I", @"Invalid unicode character value ", cp));
  }
}

- (void)dealloc {
  RELEASE_(safeOctets_);
  [super dealloc];
}
...
+ (void)initialize {
  if (self == [ComGoogleCommonNetPercentEscaper class]) {
    JreStrongAssignAndConsume(&ComGoogleCommonNetPercentEscaper_PLUS_SIGN, [IOSCharArray newArrayWithChars:(jchar[]){ '+' } count:1]);
    JreStrongAssign(&ComGoogleCommonNetPercentEscaper_UPPER_HEX_DIGITS, [@"0123456789ABCDEF" java_toCharArray]);
...
void ComGoogleCommonNetPercentEscaper_initWithNSString_withBoolean_(ComGoogleCommonNetPercentEscaper *self, NSString *safeChars, jboolean plusForSpace) {
  ...
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(safeChars);
  if ([((NSString *) nil_chk(safeChars)) java_matches:@".*[0-9A-Za-z].*"]) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"Alphanumeric characters are always 'safe' and should not be explicitly specified");
  }
  JreStrAppend(&safeChars, "$", @"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789");
  if (plusForSpace && [safeChars java_contains:@" "]) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(@"plusForSpace cannot be specified when space is a 'safe' character");
  }
  self->plusForSpace_ = plusForSpace;
  JreStrongAssign(&self->safeOctets_, ComGoogleCommonNetPercentEscaper_createSafeOctetsWithNSString_(safeChars));
}
...
IOSBooleanArray *ComGoogleCommonNetPercentEscaper_createSafeOctetsWithNSString_(NSString *safeChars) {
  ...
  IOSBooleanArray *octets = [IOSBooleanArray arrayWithLength:maxChar + 1];
  {
    IOSCharArray *a__ = safeCharArray;
    jchar const *b__ = ((IOSCharArray *) nil_chk(a__))->buffer_;
    jchar const *e__ = b__ + a__->size_;
