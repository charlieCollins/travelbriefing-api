...
//  source: ../target/dependency/com/google/common/cache/Striped64.java
//
...
- (void)retryUpdateWithLong:(jlong)x
  ...
  jint h;
  if (hc == nil) {
    [((JavaLangThreadLocal *) nil_chk(ComGoogleCommonCacheStriped64_threadHashCode)) setWithId:hc = [IOSIntArray arrayWithLength:1]];
    ...
    if ((as = JreLoadVolatileId(&cells_)) != nil && (n = ((IOSObjectArray *) nil_chk(as))->size_) > 0) {
      if ((a = IOSObjectArray_Get(nil_chk(as), (n - 1) & h)) == nil) {
        if (JreLoadVolatileInt(&busy_) == 0) {
          ComGoogleCommonCacheStriped64_Cell *r = create_ComGoogleCommonCacheStriped64_Cell_initWithLong_(x);
          if (JreLoadVolatileInt(&busy_) == 0 && ComGoogleCommonCacheStriped64_casBusy(self)) {
            ...
            @try {
              ...
              if ((rs = JreLoadVolatileId(&cells_)) != nil && (m = ((IOSObjectArray *) nil_chk(rs))->size_) > 0 && IOSObjectArray_Get(rs, j = (m - 1) & h) == nil) {
                IOSObjectArray_Set(nil_chk(rs), j, r);
                created = true;
              }
            }
            @finally {
        ...
        if (ComGoogleCommonCacheStriped64_Cell_casWithLong_withLong_(nil_chk(a), unseq$1, [self fnWithLong:v withLong:x])) break;
          ...
          @try {
            if (JreLoadVolatileId(&cells_) == as) {
              IOSObjectArray *rs = [IOSObjectArray arrayWithLength:JreLShift32(n, 1) type:ComGoogleCommonCacheStriped64_Cell_class_()];
              for (jint i = 0; i < n; ++i) IOSObjectArray_Set(rs, i, IOSObjectArray_Get(as, i));
              JreVolatileStrongAssign(&cells_, rs);
            }
          }
          @finally {
            JreAssignVolatileInt(&busy_, 0);
      ...
      *IOSIntArray_GetRef(hc, 0) = h;
      ...
      @try {
        if (JreLoadVolatileId(&cells_) == as) {
          IOSObjectArray *rs = [IOSObjectArray arrayWithLength:2 type:ComGoogleCommonCacheStriped64_Cell_class_()];
          IOSObjectArray_SetAndConsume(rs, h & 1, new_ComGoogleCommonCacheStriped64_Cell_initWithLong_(x));
          JreVolatileStrongAssign(&cells_, rs);
          init_ = true;
        }
      }
...
+ (void)initialize {
  if (self == [ComGoogleCommonCacheStriped64 class]) {
    JreStrongAssignAndConsume(&ComGoogleCommonCacheStriped64_threadHashCode, new_JavaLangThreadLocal_init());
    JreStrongAssignAndConsume(&ComGoogleCommonCacheStriped64_rng, new_JavaUtilRandom_init());
    ComGoogleCommonCacheStriped64_NCPU = [((JavaLangRuntime *) nil_chk(JavaLangRuntime_getRuntime())) availableProcessors];
    {
      @try {
        JreStrongAssign(&ComGoogleCommonCacheStriped64_UNSAFE, ComGoogleCommonCacheStriped64_getUnsafe());
      ...
      @catch (JavaLangException *e) {
        @throw create_JavaLangError_initWithNSException_(e);
      }
    }
...
SunMiscUnsafe *ComGoogleCommonCacheStriped64_getUnsafe() {
  ...
  @try {
    return JavaSecurityAccessController_doPrivilegedWithJavaSecurityPrivilegedExceptionAction_(create_ComGoogleCommonCacheStriped64_1_init());
  }
...
+ (void)initialize {
  if (self == [ComGoogleCommonCacheStriped64_Cell class]) {
    {
      @try {
        JreStrongAssign(&ComGoogleCommonCacheStriped64_Cell_UNSAFE, ComGoogleCommonCacheStriped64_getUnsafe());
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcClassInfo _ComGoogleCommonCacheStriped64_1 = { "", "com.google.common.cache", ptrTable, methods, NULL, 7, 0x8008, 2, 0, 1, -1, 2, 3, -1 };
