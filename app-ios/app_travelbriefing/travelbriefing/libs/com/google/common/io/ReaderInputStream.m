...
//  source: ../target/dependency/com/google/common/io/ReaderInputStream.java
//
...
void ComGoogleCommonIoReaderInputStream_initWithJavaIoReader_withJavaNioCharsetCharsetEncoder_withInt_(ComGoogleCommonIoReaderInputStream *self, JavaIoReader *reader, JavaNioCharsetCharsetEncoder *encoder, jint bufferSize) {
  ...
  JreStrongAssign(&self->reader_, ComGoogleCommonBasePreconditions_checkNotNullWithId_(reader));
  JreStrongAssign(&self->encoder_, ComGoogleCommonBasePreconditions_checkNotNullWithId_(encoder));
  ...
  [((JavaNioCharsetCharsetEncoder *) nil_chk(encoder)) reset];
  JreStrongAssign(&self->charBuffer_, JavaNioCharBuffer_allocateWithInt_(bufferSize));
  [((JavaNioCharBuffer *) nil_chk(self->charBuffer_)) flip];
  JreStrongAssign(&self->byteBuffer_, JavaNioByteBuffer_allocateWithInt_(bufferSize));
}
...
JavaNioCharBuffer *ComGoogleCommonIoReaderInputStream_growWithJavaNioCharBuffer_(JavaNioCharBuffer *buf) {
  ...
  [((JavaNioCharBuffer *) nil_chk(bigger)) positionWithInt:[buf position]];
  [bigger limitWithInt:[buf limit]];
  return bigger;
}
...
void ComGoogleCommonIoReaderInputStream_readMoreChars(ComGoogleCommonIoReaderInputStream *self) {
  if (ComGoogleCommonIoReaderInputStream_availableCapacityWithJavaNioBuffer_(self->charBuffer_) == 0) {
    if ([((JavaNioCharBuffer *) nil_chk(self->charBuffer_)) position] > 0) {
      [((JavaNioCharBuffer *) nil_chk([((JavaNioCharBuffer *) nil_chk(self->charBuffer_)) compact])) flip];
    }
    else {
      JreStrongAssign(&self->charBuffer_, ComGoogleCommonIoReaderInputStream_growWithJavaNioCharBuffer_(self->charBuffer_));
    }
  }
  ...
  if (numChars == -1) {
    ...
    [((JavaNioCharBuffer *) nil_chk(self->charBuffer_)) limitWithInt:limit + numChars];
  }
}
...
void ComGoogleCommonIoReaderInputStream_startDrainingWithBoolean_(ComGoogleCommonIoReaderInputStream *self, jboolean overflow) {
  [((JavaNioByteBuffer *) nil_chk(self->byteBuffer_)) flip];
  if (overflow && [((JavaNioByteBuffer *) nil_chk(self->byteBuffer_)) remaining] == 0) {
    JreStrongAssign(&self->byteBuffer_, JavaNioByteBuffer_allocateWithInt_([((JavaNioByteBuffer *) nil_chk(self->byteBuffer_)) capacity] * 2));
  }
  else {
    self->draining_ = true;
  }
...
jint ComGoogleCommonIoReaderInputStream_drainWithByteArray_withInt_withInt_(ComGoogleCommonIoReaderInputStream *self, IOSByteArray *b, jint off, jint len) {
  ...
  [((JavaNioByteBuffer *) nil_chk(self->byteBuffer_)) getWithByteArray:b withInt:off withInt:remaining];
  return remaining;
}
