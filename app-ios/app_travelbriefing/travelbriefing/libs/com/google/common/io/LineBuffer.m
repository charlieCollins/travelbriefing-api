...
//  source: ../target/dependency/com/google/common/io/LineBuffer.java
//
...
- (void)addWithCharArray:(IOSCharArray *)cbuf
  ...
  for (jint end = off + len; pos < end; pos++) {
    switch (IOSCharArray_Get(nil_chk(cbuf), pos)) {
      case 0x000d:
      [((JavaLangStringBuilder *) nil_chk(line_)) appendWithCharArray:cbuf withInt:start withInt:pos - start];
      sawReturn_ = true;
      if (pos + 1 < end) {
  ...
  [((JavaLangStringBuilder *) nil_chk(line_)) appendWithCharArray:cbuf withInt:start withInt:off + len - start];
}
...
- (void)finish {
  if (sawReturn_ || [((JavaLangStringBuilder *) nil_chk(line_)) length] > 0) {
...
void ComGoogleCommonIoLineBuffer_init(ComGoogleCommonIoLineBuffer *self) {
  NSObject_init(self);
  JreStrongAssignAndConsume(&self->line_, new_JavaLangStringBuilder_init());
}
