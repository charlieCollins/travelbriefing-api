...
//  source: ../target/dependency/com/google/common/hash/Murmur3_32HashFunction.java
//
...
- (id<ComGoogleCommonHashHasher>)newHasher {
  return create_ComGoogleCommonHashMurmur3_32HashFunction_Murmur3_32Hasher_initWithInt_(seed_);
}
...
- (ComGoogleCommonHashHashCode *)hashUnencodedCharsWithJavaLangCharSequence:(id<JavaLangCharSequence>)input {
  jint h1 = seed_;
  for (jint i = 1; i < [((id<JavaLangCharSequence>) nil_chk(input)) length]; i += 2) {
  ...
  if (([input length] & 1) == 1) {
    jint k1 = [input charAtWithInt:[input length] - 1];
    k1 = ComGoogleCommonHashMurmur3_32HashFunction_mixK1WithInt_(k1);
    h1 ^= k1;
  }
  return ComGoogleCommonHashMurmur3_32HashFunction_fmixWithInt_withInt_(h1, ComGoogleCommonPrimitivesChars_BYTES * [input length]);
}
