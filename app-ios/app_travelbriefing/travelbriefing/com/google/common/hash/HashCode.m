...
//  source: ../target/dependency/com/google/common/hash/HashCode.java
//
...
- (jint)writeBytesToWithByteArray:(IOSByteArray *)dest
                          withInt:(jint)offset
                          withInt:(jint)maxLength {
  maxLength = ComGoogleCommonPrimitivesInts_minWithIntArray_([IOSIntArray arrayWithInts:(jint[]){ maxLength, [self bits] / 8 } count:2]);
...
- (NSString *)description {
  IOSByteArray *bytes = [self getBytesInternal];
  JavaLangStringBuilder *sb = create_JavaLangStringBuilder_initWithInt_(2 * ((IOSByteArray *) nil_chk(bytes))->size_);
  {
    ...
    while (b__ < e__) {
      jbyte b = *b__++;
      [((JavaLangStringBuilder *) nil_chk([sb appendWithChar:IOSCharArray_Get(nil_chk(ComGoogleCommonHashHashCode_hexDigits), (JreRShift32(b, 4)) & (jint) 0xf)])) appendWithChar:IOSCharArray_Get(ComGoogleCommonHashHashCode_hexDigits, b & (jint) 0xf)];
    }
  }
...
+ (void)initialize {
  if (self == [ComGoogleCommonHashHashCode class]) {
    JreStrongAssign(&ComGoogleCommonHashHashCode_hexDigits, [@"0123456789abcdef" java_toCharArray]);
...
ComGoogleCommonHashHashCode *ComGoogleCommonHashHashCode_fromIntWithInt_(jint hash_) {
  ...
  return create_ComGoogleCommonHashHashCode_IntHashCode_initWithInt_(hash_);
}

ComGoogleCommonHashHashCode *ComGoogleCommonHashHashCode_fromLongWithLong_(jlong hash_) {
  ComGoogleCommonHashHashCode_initialize();
  return create_ComGoogleCommonHashHashCode_LongHashCode_initWithLong_(hash_);
}
...
ComGoogleCommonHashHashCode *ComGoogleCommonHashHashCode_fromBytesNoCopyWithByteArray_(IOSByteArray *bytes) {
  ComGoogleCommonHashHashCode_initialize();
  return create_ComGoogleCommonHashHashCode_BytesHashCode_initWithByteArray_(bytes);
}

ComGoogleCommonHashHashCode *ComGoogleCommonHashHashCode_fromStringWithNSString_(NSString *string) {
  ComGoogleCommonHashHashCode_initialize();
  ComGoogleCommonBasePreconditions_checkArgumentWithBoolean_withNSString_withId_(((jint) [((NSString *) nil_chk(string)) length]) >= 2, @"input string (%s) must have at least 2 characters", string);
  ComGoogleCommonBasePreconditions_checkArgumentWithBoolean_withNSString_withId_(((jint) [string length]) % 2 == 0, @"input string (%s) must have an even number of characters", string);
  IOSByteArray *bytes = [IOSByteArray arrayWithLength:((jint) [string length]) / 2];
  for (jint i = 0; i < ((jint) [string length]); i += 2) {
...
jint ComGoogleCommonHashHashCode_decodeWithChar_(jchar ch) {
  ...
  @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$C", @"Illegal hexadecimal character: ", ch));
}
...
- (IOSByteArray *)asBytes {
  return [IOSByteArray arrayWithBytes:(jbyte[]){ (jbyte) hash__, (jbyte) (JreRShift32(hash__, 8)), (jbyte) (JreRShift32(hash__, 16)), (jbyte) (JreRShift32(hash__, 24)) } count:4];
}
...
- (jlong)asLong {
  @throw create_JavaLangIllegalStateException_initWithNSString_(@"this HashCode only has 32 bits; cannot create a long");
}
...
- (IOSByteArray *)asBytes {
  return [IOSByteArray arrayWithBytes:(jbyte[]){ (jbyte) hash__, (jbyte) (JreRShift64(hash__, 8)), (jbyte) (JreRShift64(hash__, 16)), (jbyte) (JreRShift64(hash__, 24)), (jbyte) (JreRShift64(hash__, 32)), (jbyte) (JreRShift64(hash__, 40)), (jbyte) (JreRShift64(hash__, 48)), (jbyte) (JreRShift64(hash__, 56)) } count:8];
}
...
void ComGoogleCommonHashHashCode_BytesHashCode_initWithByteArray_(ComGoogleCommonHashHashCode_BytesHashCode *self, IOSByteArray *bytes) {
  ComGoogleCommonHashHashCode_init(self);
  JreStrongAssign(&self->bytes_, ComGoogleCommonBasePreconditions_checkNotNullWithId_(bytes));
}
