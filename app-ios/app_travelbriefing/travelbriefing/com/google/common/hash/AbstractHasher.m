...
//  source: ../target/dependency/com/google/common/hash/AbstractHasher.java
//
...
- (id<ComGoogleCommonHashHasher>)putUnencodedCharsWithJavaLangCharSequence:(id<JavaLangCharSequence>)charSequence {
  for (jint i = 0, len = [((id<JavaLangCharSequence>) nil_chk(charSequence)) length]; i < len; i++) {
    [self putCharWithChar:[charSequence charAtWithInt:i]];
  }
  return self;
}
