...
//  source: ../target/dependency/com/google/common/primitives/UnsignedLongs.java
//
...
+ (void)initialize {
  if (self == [ComGoogleCommonPrimitivesUnsignedLongs class]) {
    JreStrongAssignAndConsume(&ComGoogleCommonPrimitivesUnsignedLongs_maxValueDivs, [IOSLongArray newArrayWithLength:JavaLangCharacter_MAX_RADIX + 1]);
    JreStrongAssignAndConsume(&ComGoogleCommonPrimitivesUnsignedLongs_maxValueMods, [IOSIntArray newArrayWithLength:JavaLangCharacter_MAX_RADIX + 1]);
    JreStrongAssignAndConsume(&ComGoogleCommonPrimitivesUnsignedLongs_maxSafeDigits, [IOSIntArray newArrayWithLength:JavaLangCharacter_MAX_RADIX + 1]);
    {
      JavaMathBigInteger *overflow = create_JavaMathBigInteger_initWithNSString_withInt_(@"10000000000000000", 16);
      for (jint i = JavaLangCharacter_MIN_RADIX; i <= JavaLangCharacter_MAX_RADIX; i++) {
        ...
        *IOSIntArray_GetRef(ComGoogleCommonPrimitivesUnsignedLongs_maxSafeDigits, i) = ((jint) [((NSString *) nil_chk([overflow toStringWithInt:i])) length]) - 1;
      }
    }
...
NSString *ComGoogleCommonPrimitivesUnsignedLongs_joinWithNSString_withLongArray_(NSString *separator, IOSLongArray *array) {
  ...
  JavaLangStringBuilder *builder = create_JavaLangStringBuilder_initWithInt_(array->size_ * 5);
  [builder appendWithNSString:ComGoogleCommonPrimitivesUnsignedLongs_toStringWithLong_(IOSLongArray_Get(array, 0))];
  for (jint i = 1; i < array->size_; i++) {
    [((JavaLangStringBuilder *) nil_chk([builder appendWithNSString:separator])) appendWithNSString:ComGoogleCommonPrimitivesUnsignedLongs_toStringWithLong_(IOSLongArray_Get(array, i))];
  }
...
jlong ComGoogleCommonPrimitivesUnsignedLongs_decodeWithNSString_(NSString *stringValue) {
  ...
  @catch (JavaLangNumberFormatException *e) {
    JavaLangNumberFormatException *decodeException = create_JavaLangNumberFormatException_initWithNSString_(JreStrcat("$$", @"Error parsing value: ", stringValue));
    [decodeException initCauseWithNSException:e];
...
jlong ComGoogleCommonPrimitivesUnsignedLongs_parseUnsignedLongWithNSString_withInt_(NSString *string, jint radix) {
  ...
  if (((jint) [((NSString *) nil_chk(string)) length]) == 0) {
  ...
  if (radix < JavaLangCharacter_MIN_RADIX || radix > JavaLangCharacter_MAX_RADIX) {
    @throw create_JavaLangNumberFormatException_initWithNSString_(JreStrcat("$I", @"illegal radix: ", radix));
  }
  jint maxSafePos = IOSIntArray_Get(nil_chk(ComGoogleCommonPrimitivesUnsignedLongs_maxSafeDigits), radix) - 1;
  jlong value = 0;
  for (jint pos = 0; pos < ((jint) [string length]); pos++) {
    jint digit = JavaLangCharacter_digitWithChar_withInt_([string charAtWithInt:pos], radix);
    if (digit == -1) {
      @throw create_JavaLangNumberFormatException_initWithNSString_(string);
    }
    if (pos > maxSafePos && ComGoogleCommonPrimitivesUnsignedLongs_overflowInParseWithLong_withInt_withInt_(value, digit, radix)) {
      @throw create_JavaLangNumberFormatException_initWithNSString_(JreStrcat("$$", @"Too large for unsigned long: ", string));
    }
...
NSString *ComGoogleCommonPrimitivesUnsignedLongs_toStringWithLong_withInt_(jlong x, jint radix) {
  ...
  if (x == 0) {
    ...
    IOSCharArray *buf = [IOSCharArray arrayWithLength:64];
    jint i = buf->size_;
    if ((radix & (radix - 1)) == 0) {
...
+ (void)initialize {
  if (self == [ComGoogleCommonPrimitivesUnsignedLongs_LexicographicalComparator class]) {
    ...
    for (jint i = 0; i < 1; i++) {
      (ComGoogleCommonPrimitivesUnsignedLongs_LexicographicalComparator_values_[i] = e = objc_constructInstance(self, (void *)ptr), ptr += objSize);
      ComGoogleCommonPrimitivesUnsignedLongs_LexicographicalComparator_initWithNSString_withInt_(e, names[i], i);
    }
