...
//  source: ../target/dependency/com/google/common/util/concurrent/RateLimiter.java
//
...
- (NSString *)description {
  return NSString_java_formatWithJavaUtilLocale_withNSString_withNSObjectArray_(JreLoadStatic(JavaUtilLocale, ROOT), @"RateLimiter[stableRate=%3.1fqps]", [IOSObjectArray arrayWithObjects:(id[]){ JavaLangDouble_valueOfWithDouble_(ComGoogleCommonUtilConcurrentRateLimiter_getRate(self)) } count:1 type:NSObject_class_()]);
}
...
ComGoogleCommonUtilConcurrentRateLimiter *ComGoogleCommonUtilConcurrentRateLimiter_createWithComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_withDouble_(ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch *stopwatch, jdouble permitsPerSecond) {
  ComGoogleCommonUtilConcurrentRateLimiter_initialize();
  ComGoogleCommonUtilConcurrentRateLimiter *rateLimiter = create_ComGoogleCommonUtilConcurrentSmoothRateLimiter_SmoothBursty_initWithComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_withDouble_(stopwatch, 1.0);
...
ComGoogleCommonUtilConcurrentRateLimiter *ComGoogleCommonUtilConcurrentRateLimiter_createWithComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_withDouble_withLong_withJavaUtilConcurrentTimeUnit_withDouble_(ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch *stopwatch, jdouble permitsPerSecond, jlong warmupPeriod, JavaUtilConcurrentTimeUnit *unit, jdouble coldFactor) {
  ComGoogleCommonUtilConcurrentRateLimiter_initialize();
  ComGoogleCommonUtilConcurrentRateLimiter *rateLimiter = create_ComGoogleCommonUtilConcurrentSmoothRateLimiter_SmoothWarmingUp_initWithComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_withLong_withJavaUtilConcurrentTimeUnit_withDouble_(stopwatch, warmupPeriod, unit, coldFactor);
  ComGoogleCommonUtilConcurrentRateLimiter_setRateWithDouble_(rateLimiter, permitsPerSecond);
  return rateLimiter;
}
...
id ComGoogleCommonUtilConcurrentRateLimiter_mutex(ComGoogleCommonUtilConcurrentRateLimiter *self) {
  ...
  if (mutex == nil) {
    @synchronized(self) {
      ...
      if (mutex == nil) {
        JreVolatileStrongAssign(&self->mutexDoNotUseDirectly_, mutex = create_NSObject_init());
      }
    }
  }
  return mutex;
...
void ComGoogleCommonUtilConcurrentRateLimiter_initWithComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_(ComGoogleCommonUtilConcurrentRateLimiter *self, ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch *stopwatch) {
  NSObject_init(self);
  JreStrongAssign(&self->stopwatch_, ComGoogleCommonBasePreconditions_checkNotNullWithId_(stopwatch));
}
...
ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch *ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_createFromSystemTimer() {
  ...
  return create_ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_1_init();
}
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcClassInfo _ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_1 = { "", "com.google.common.util.concurrent", ptrTable, methods, fields, 7, 0x8008, 3, 1, 2, -1, 3, -1, -1 };
...
void ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_1_init(ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_1 *self) {
  ComGoogleCommonUtilConcurrentRateLimiter_SleepingStopwatch_init(self);
  JreStrongAssign(&self->stopwatch_, ComGoogleCommonBaseStopwatch_createStarted());
}
