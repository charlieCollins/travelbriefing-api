...
//  source: ../target/dependency/com/google/common/io/CharSequenceReader.java
//
...
- (jint)readWithJavaNioCharBuffer:(JavaNioCharBuffer *)target {
  @synchronized(self) {
    ...
    for (jint i = 0; i < charsToRead; i++) {
      [target putWithChar:[((id<JavaLangCharSequence>) nil_chk(seq_)) charAtWithInt:pos_++]];
    }
    return charsToRead;
  }
}
...
void ComGoogleCommonIoCharSequenceReader_checkOpen(ComGoogleCommonIoCharSequenceReader *self) {
  if (self->seq_ == nil) {
    @throw create_JavaIoIOException_initWithNSString_(@"reader closed");
  }
}
...
jint ComGoogleCommonIoCharSequenceReader_remaining(ComGoogleCommonIoCharSequenceReader *self) {
  return [((id<JavaLangCharSequence>) nil_chk(self->seq_)) length] - self->pos_;
}
