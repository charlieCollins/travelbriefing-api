...
//  source: ../target/dependency/com/google/common/io/LineReader.java
//
...
- (NSString *)readLine {
  while ([((id<JavaUtilQueue>) nil_chk(lines_)) peek] == nil) {
    [((JavaNioCharBuffer *) nil_chk(cbuf_)) clear];
...
void ComGoogleCommonIoLineReader_initWithJavaLangReadable_(ComGoogleCommonIoLineReader *self, id<JavaLangReadable> readable) {
  NSObject_init(self);
  JreStrongAssign(&self->cbuf_, ComGoogleCommonIoCharStreams_createBuffer());
  JreStrongAssign(&self->buf_, [((JavaNioCharBuffer *) nil_chk(self->cbuf_)) array]);
  JreStrongAssignAndConsume(&self->lines_, new_JavaUtilLinkedList_init());
  JreStrongAssignAndConsume(&self->lineBuf_, new_ComGoogleCommonIoLineReader_1_initWithComGoogleCommonIoLineReader_(self));
  JreStrongAssign(&self->readable_, ComGoogleCommonBasePreconditions_checkNotNullWithId_(readable));
  JreStrongAssign(&self->reader_, ([readable isKindOfClass:[JavaIoReader class]]) ? (JavaIoReader *) cast_chk(readable, [JavaIoReader class]) : nil);
}
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcClassInfo _ComGoogleCommonIoLineReader_1 = { "", "com.google.common.io", ptrTable, methods, fields, 7, 0x8008, 2, 1, 2, -1, -1, -1, -1 };
