...
//  source: ../target/dependency/com/google/common/base/Ascii.java
//
...
NSString *ComGoogleCommonBaseAscii_toLowerCaseWithJavaLangCharSequence_(id<JavaLangCharSequence> chars) {
  ...
  IOSCharArray *newChars = [IOSCharArray arrayWithLength:[((id<JavaLangCharSequence>) nil_chk(chars)) length]];
...
NSString *ComGoogleCommonBaseAscii_truncateWithJavaLangCharSequence_withInt_withNSString_(id<JavaLangCharSequence> seq, jint maxLength, NSString *truncationIndicator) {
  ComGoogleCommonBaseAscii_initialize();
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(seq);
  jint truncationLength = maxLength - ((jint) [((NSString *) nil_chk(truncationIndicator)) length]);
  ComGoogleCommonBasePreconditions_checkArgumentWithBoolean_withNSString_withInt_withInt_(truncationLength >= 0, @"maxLength (%s) must be >= length of the truncation indicator (%s)", maxLength, ((jint) [truncationIndicator length]));
  if ([((id<JavaLangCharSequence>) nil_chk(seq)) length] <= maxLength) {
    NSString *string = [seq description];
    if (((jint) [((NSString *) nil_chk(string)) length]) <= maxLength) {
      return string;
    }
    seq = string;
  }
  return [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([create_JavaLangStringBuilder_initWithInt_(maxLength) appendWithJavaLangCharSequence:seq withInt:0 withInt:truncationLength])) appendWithNSString:truncationIndicator])) description];
}
...
jboolean ComGoogleCommonBaseAscii_equalsIgnoreCaseWithJavaLangCharSequence_withJavaLangCharSequence_(id<JavaLangCharSequence> s1, id<JavaLangCharSequence> s2) {
  ComGoogleCommonBaseAscii_initialize();
  jint length = [((id<JavaLangCharSequence>) nil_chk(s1)) length];
  if (s1 == s2) {
    return true;
  }
  if (length != [((id<JavaLangCharSequence>) nil_chk(s2)) length]) {
    return false;
  }
  for (jint i = 0; i < length; i++) {
