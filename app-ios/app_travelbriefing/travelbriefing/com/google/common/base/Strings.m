...
//  source: ../target/dependency/com/google/common/base/Strings.java
//
...
NSString *ComGoogleCommonBaseStrings_padStartWithNSString_withInt_withChar_(NSString *string, jint minLength, jchar padChar) {
  ...
  if (((jint) [((NSString *) nil_chk(string)) length]) >= minLength) {
    return string;
  }
  JavaLangStringBuilder *sb = create_JavaLangStringBuilder_initWithInt_(minLength);
  for (jint i = ((jint) [string length]); i < minLength; i++) {
    [sb appendWithChar:padChar];
  }
...
NSString *ComGoogleCommonBaseStrings_repeatWithNSString_withInt_(NSString *string, jint count) {
  ...
  if (size != longSize) {
    @throw create_JavaLangArrayIndexOutOfBoundsException_initWithNSString_(JreStrcat("$J", @"Required array size too large: ", longSize));
  }
  IOSCharArray *array = [IOSCharArray arrayWithLength:size];
...
NSString *ComGoogleCommonBaseStrings_commonPrefixWithJavaLangCharSequence_withJavaLangCharSequence_(id<JavaLangCharSequence> a, id<JavaLangCharSequence> b) {
  ...
  jint maxPrefixLength = JavaLangMath_minWithInt_withInt_([((id<JavaLangCharSequence>) nil_chk(a)) length], [((id<JavaLangCharSequence>) nil_chk(b)) length]);
  jint p = 0;
...
NSString *ComGoogleCommonBaseStrings_commonSuffixWithJavaLangCharSequence_withJavaLangCharSequence_(id<JavaLangCharSequence> a, id<JavaLangCharSequence> b) {
  ...
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(a);
  ComGoogleCommonBasePreconditions_checkNotNullWithId_(b);
  jint maxSuffixLength = JavaLangMath_minWithInt_withInt_([((id<JavaLangCharSequence>) nil_chk(a)) length], [((id<JavaLangCharSequence>) nil_chk(b)) length]);
  jint s = 0;
  while (s < maxSuffixLength && [a charAtWithInt:[a length] - s - 1] == [b charAtWithInt:[b length] - s - 1]) {
    s++;
  }
  if (ComGoogleCommonBaseStrings_validSurrogatePairAtWithJavaLangCharSequence_withInt_(a, [a length] - s - 1) || ComGoogleCommonBaseStrings_validSurrogatePairAtWithJavaLangCharSequence_withInt_(b, [b length] - s - 1)) {
    s--;
  }
  return [((id<JavaLangCharSequence>) nil_chk([a subSequenceFrom:[a length] - s to:[a length]])) description];
}
...
jboolean ComGoogleCommonBaseStrings_validSurrogatePairAtWithJavaLangCharSequence_withInt_(id<JavaLangCharSequence> string, jint index) {
  ComGoogleCommonBaseStrings_initialize();
  return index >= 0 && index <= ([((id<JavaLangCharSequence>) nil_chk(string)) length] - 2) && JavaLangCharacter_isHighSurrogateWithChar_([string charAtWithInt:index]) && JavaLangCharacter_isLowSurrogateWithChar_([string charAtWithInt:index + 1]);
}
