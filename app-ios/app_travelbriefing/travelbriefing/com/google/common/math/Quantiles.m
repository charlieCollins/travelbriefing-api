...
//  source: ../target/dependency/com/google/common/math/Quantiles.java
//
...
ComGoogleCommonMathQuantiles_Scale *ComGoogleCommonMathQuantiles_scale__WithInt_(jint scale_) {
  ComGoogleCommonMathQuantiles_initialize();
  return create_ComGoogleCommonMathQuantiles_Scale_initWithInt_(scale_);
}
...
void ComGoogleCommonMathQuantiles_checkIndexWithInt_withInt_(jint index, jint scale_) {
  ...
  if (index < 0 || index > scale_) {
    @throw create_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I", @"Quantile indexes must be between 0 and the scale, which is ", scale_));
  }
}
...
IOSDoubleArray *ComGoogleCommonMathQuantiles_longsToDoublesWithLongArray_(IOSLongArray *longs) {
  ...
  IOSDoubleArray *doubles = [IOSDoubleArray arrayWithLength:len];
...
- (ComGoogleCommonMathQuantiles_ScaleAndIndex *)indexWithInt:(jint)index {
  return create_ComGoogleCommonMathQuantiles_ScaleAndIndex_initWithInt_withInt_(scale__, index);
}

- (ComGoogleCommonMathQuantiles_ScaleAndIndexes *)indexesWithIntArray:(IOSIntArray *)indexes {
  return create_ComGoogleCommonMathQuantiles_ScaleAndIndexes_initWithInt_withIntArray_(scale__, [((IOSIntArray *) nil_chk(indexes)) java_clone]);
}

- (ComGoogleCommonMathQuantiles_ScaleAndIndexes *)indexesWithJavaUtilCollection:(id<JavaUtilCollection>)indexes {
  return create_ComGoogleCommonMathQuantiles_ScaleAndIndexes_initWithInt_withIntArray_(scale__, ComGoogleCommonPrimitivesInts_toArrayWithJavaUtilCollection_(indexes));
}
...
- (id<JavaUtilMap>)computeInPlaceWithDoubleArray:(IOSDoubleArray *)dataset {
  ComGoogleCommonBasePreconditions_checkArgumentWithBoolean_withId_(((IOSDoubleArray *) nil_chk(dataset))->size_ > 0, @"Cannot calculate quantiles of an empty dataset");
  if (ComGoogleCommonMathQuantiles_containsNaNWithDoubleArray_(dataset)) {
    id<JavaUtilMap> nanMap = create_JavaUtilHashMap_init();
    {
      ...
      while (b__ < e__) {
        jint index = *b__++;
        [nanMap putWithId:JavaLangInteger_valueOfWithInt_(index) withId:JavaLangDouble_valueOfWithDouble_(JavaLangDouble_NaN)];
      }
    }
    return JavaUtilCollections_unmodifiableMapWithJavaUtilMap_(nanMap);
  }
  IOSIntArray *quotients = [IOSIntArray arrayWithLength:((IOSIntArray *) nil_chk(indexes_))->size_];
  IOSIntArray *remainders = [IOSIntArray arrayWithLength:indexes_->size_];
  IOSIntArray *requiredSelections = [IOSIntArray arrayWithLength:indexes_->size_ * 2];
  ...
  id<JavaUtilMap> ret = create_JavaUtilHashMap_init();
  for (jint i = 0; i < indexes_->size_; i++) {
    jint quotient = IOSIntArray_Get(quotients, i);
    jint remainder = IOSIntArray_Get(remainders, i);
    if (remainder == 0) {
      [ret putWithId:JavaLangInteger_valueOfWithInt_(IOSIntArray_Get(indexes_, i)) withId:JavaLangDouble_valueOfWithDouble_(IOSDoubleArray_Get(dataset, quotient))];
    }
    else {
      [ret putWithId:JavaLangInteger_valueOfWithInt_(IOSIntArray_Get(indexes_, i)) withId:JavaLangDouble_valueOfWithDouble_(ComGoogleCommonMathQuantiles_interpolateWithDouble_withDouble_withDouble_withDouble_(IOSDoubleArray_Get(dataset, quotient), IOSDoubleArray_Get(dataset, quotient + 1), remainder, scale__))];
    }
  }
...
void ComGoogleCommonMathQuantiles_ScaleAndIndexes_initWithInt_withIntArray_(ComGoogleCommonMathQuantiles_ScaleAndIndexes *self, jint scale_, IOSIntArray *indexes) {
  ...
  self->scale__ = scale_;
  JreStrongAssign(&self->indexes_, indexes);
}
